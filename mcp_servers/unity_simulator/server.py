...
def _handle_apply_demolition(arguments: dict) -> dict:
    failed_elements = arguments.get("failed_elements", [])
    if not failed_elements:
        return {"error": "At least one failed_element ID is required"}
...
def _handle_reset(arguments: dict) -> dict:
...
async def call_tool(name: str, arguments: dict) -> list[TextContent]:
    try:
        if name == "apply_demolition_action":
            ...
            result = await asyncio.wait_for(
                asyncio.to_thread(_handle_reset, arguments),
                timeout=15.0,
            )
            return [TextContent(type="text", text=json.dumps(result, indent=2))]
        ...
        return [TextContent(type="text", text=json.dumps({"error": "Unity simulator action timed out (>15s)"}))]
    except Exception as e:
