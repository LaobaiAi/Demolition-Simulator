...
def make_server_config(name: str) -> dict:
    ...
    caiao_servers_dir = os.path.join(
    ...
    return {
        ...
        "args": ["server.py"],
        "cwd": os.path.join(caiao_servers_dir, "demo_calculator"),
    }
...
async def test_hub_start_and_stop():
    ...
    hub = CAIAOClientHub([make_server_config("test_calc")])
    await hub.start_all()
